...
// Fungsi badge warna (sama seperti kategori)
...
// Proses tambah (dengan validasi duplikat nama & telepon, alamat & telepon wajib)
if ($_SERVER['REQUEST_METHOD'] == 'POST' && isset($_POST['tambah'])) {
    ...
    if (empty($alamat)) {
        $_SESSION['error'] = "Alamat supplier harus diisi.";
        header("Location: master_supplier.php");
        exit;
    }
    if (empty($telepon)) {
        $_SESSION['error'] = "Nomor telepon supplier harus diisi.";
        header("Location: master_supplier.php");
        exit;
    }
    
    // Cek duplikasi nama (case-insensitive)
    $cekNama = $conn->prepare("SELECT id FROM supplier WHERE LOWER(nama) = LOWER(?)");
    $cekNama->bind_param("s", $nama);
    $cekNama->execute();
    if ($cekNama->get_result()->num_rows > 0) {
        $_SESSION['error'] = "Nama supplier sudah terdaftar!";
        header("Location: master_supplier.php");
        exit;
    }
    
    // Cek duplikasi telepon
    $cekTelepon = $conn->prepare("SELECT id FROM supplier WHERE telepon = ?");
    $cekTelepon->bind_param("s", $telepon);
    $cekTelepon->execute();
    if ($cekTelepon->get_result()->num_rows > 0) {
        $_SESSION['error'] = "Nomor telepon sudah digunakan oleh supplier lain!";
        header("Location: master_supplier.php");
        exit;
    }
...
// Proses edit (dengan validasi duplikat nama & telepon kecuali diri sendiri, alamat & telepon wajib)
if ($_SERVER['REQUEST_METHOD'] == 'POST' && isset($_POST['edit'])) {
    ...
    $cekNama = $conn->prepare("SELECT id FROM supplier WHERE LOWER(nama) = LOWER(?) AND id != ?");
    $cekNama->bind_param("si", $nama, $id);
    $cekNama->execute();
    if ($cekNama->get_result()->num_rows > 0) {
        $_SESSION['error'] = "Nama supplier sudah digunakan oleh supplier lain!";
        header("Location: master_supplier.php");
        exit;
    }
    
    // Cek duplikasi telepon (kecuali dirinya sendiri)
    $cekTelepon = $conn->prepare("SELECT id FROM supplier WHERE telepon = ? AND id != ?");
    $cekTelepon->bind_param("si", $telepon, $id);
    $cekTelepon->execute();
    if ($cekTelepon->get_result()->num_rows > 0) {
        $_SESSION['error'] = "Nomor telepon sudah digunakan oleh supplier lain!";
        header("Location: master_supplier.php");
...
// Proses hapus (dengan prepared statement)
if (isset($_GET['hapus'])) {
    $id = (int)$_GET['hapus'];
    $stmt = $conn->prepare("SELECT id_barang FROM barang WHERE supplier_id = ? LIMIT 1");
    $stmt->bind_param("i", $id);
    $stmt->execute();
    $cek = $stmt->get_result();
    if ($cek->num_rows > 0) {
        $_SESSION['error'] = "Supplier tidak bisa dihapus karena masih digunakan oleh barang.";
    } else {
...
if (!empty($search)) {
    $stmt = $conn->prepare("
        SELECT s.*, COUNT(b.id_barang) as total_barang
        FROM supplier s
    ...
    <style>
        ...
        *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
        ...
        .btn-toggle {
            ...
            display: flex; align-items: center; justify-content: center;
            cursor: pointer; transition: var(--transition); font-size: 16px; flex-shrink: 0;
        }
        ...
        /* ─── SEMUA CSS KONTEN ASLI (TIDAK DIUBAH) ─── */
        ...
        .alert {
            ...
            font-size: 13.5px; font-weight: 450; margin-bottom: 20px;
        ...
        .card-head {
            ...
            gap: 12px; border-bottom: 1px solid var(--border); background: var(--surface-2);
        }
        .card-head-title {
        ...
        .phone-chip i { font-size: 12px; color: var(--text-muted); }
        ...
        .btn-action {
            ...
            transition: var(--transition); text-decoration: none; font-size: 14px;
        }
        ...
        .btn-cancel {
            ...
            font-weight: 500; font-family: inherit; cursor: pointer; transition: var(--transition);
        }
    ...
    <div class="page-body">
        ...
        <div class="page-header">
            <div>
                ...
                <div class="page-subtitle">Kelola data supplier untuk pengadaan barang</div>
            </div>
                    ...
                    <tbody id="tableBody">
                        <?php if($supplier->num_rows > 0): ?>
                            ...
                            <tr>
                                ...
                                <td>
                                    <span class="alamat-text" title="<?= htmlspecialchars($row['alamat'] ?? '') ?>">
                                        <?= $row['alamat'] ? htmlspecialchars($row['alamat']) : '<span style="color:var(--text-muted)">—</span>' ?>
                                    </span>
                                </td>
                                <td>
                                    <?php if($row['telepon']): ?>
...
<!-- MODAL TAMBAH (dengan required pada alamat & telepon) -->
        ...
        <form method="POST">
            ...
            <div class="modal-body">
                ...
                <div class="form-group">
                    ...
                    <textarea name="alamat" class="form-control" rows="2" placeholder="" required></textarea>
                </div>
                <div class="form-group">
...
<!-- MODAL EDIT (dengan required pada alamat & telepon) -->
...
<script>
    // ─── SIDEBAR TOGGLE (VERSI DASHBOARD) ───
